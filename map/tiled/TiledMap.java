...
import nl.tdegroot.games.pixxel.map.TileBasedMap;
...
public class TiledMap {
     ...
     *
     * @param name The name of the object layer to search for
     ...
     * @param layerIndex The index of the layer on which the tile whose image should be
     *                   retrieve exists
     * @return The image used to draw the specified tile or null if there is no
     * image for the specified tile.
     */
    public Sprite getTileSprite(int x, int y, int layerIndex) {
    ...
    public void render(int x, int y, Screen screen) {
        render(x, y, 0, 0, width, height, false, screen);
    }
     ...
     */
    public void render(int x, int y, int layer, Screen screen) {
        render(x, y, 0, 0, getWidth(), getHeight(), layer, false, screen);
    }
     ...
     */
    public void render(int x, int y, int sx, int sy, int width, int height, Screen screen) {
        render(x, y, sx, sy, width, height, false, screen);
    }
     ...
     */
    public void render(int x, int y, int sx, int sy, int width, int height, int l, boolean lineByLine, Screen screen) {
        Layer layer = (Layer) layers.get(l);

        switch (orientation) {
            case ORTHOGONAL:
                for (int ty = 0; ty < height; ty++) {
                    layer.render(x, y, sx, sy, width, ty, lineByLine, tileWidth, tileHeight, screen);
                }
                break;
            case ISOMETRIC:
                renderIsometricMap(x, y, sx, sy, width, height, layer, lineByLine, screen);
                break;
            default:
     ...
     *                   {@link #renderedLine(int, int, int)}
     */
    public void render(int x, int y, int sx, int sy, int width, int height, boolean lineByLine, Screen screen) {
        switch (orientation) {
            case ORTHOGONAL:
                for (int ty = 0; ty < height; ty++) {
                    for (int i = 0; i < layers.size(); i++) {
                ...
                break;
            case ISOMETRIC:
                renderIsometricMap(x, y, sx, sy, width, height, null, lineByLine, screen);
                break;
            default:
                // log error or something
        }
     ...
     * @param layer      if this is null all layers are rendered, if not only the
     ...
     */
    protected void renderIsometricMap(int x, int y, int sx, int sy, int width, int height, Layer layer, boolean lineByLine, Screen screen) {
        ...
        while (!allProcessed) {
            ...
            for (int burner = 0; burner <= min; currentTileX++, currentTileY--, burner++) {
                for (int layerIdx = 0; layerIdx < drawLayers.size(); layerIdx++) {
                    ...
                    currentLayer.render(currentLineX, initialLineY, currentTileX, currentTileY, 1, 0, lineByLine, tileWidth, tileHeight, screen);
                }
     ...
     */
    public int getObjectLayerCount() {
    ...
    private void load(InputStream in, String tileSetsLocation) throws GameException {
        ...
        try {
			 ...
			 * AdversaryException("Only orthogonal maps supported, found: "+orient);
			 * }
			 */
            ...
            Log.error(e);
            throw new GameException("Failed to parse tilemap", e);
        }
    }
